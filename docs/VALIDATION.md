...
전체 신호 편향 검사 결과는 [`results/2026-09-01-lookahead-analysis.md`](results/2026-09-01-lookahead-analysis.md)에 기록되어 있습니다. Vibe 전략은 표본 부족으로 제한적인 결과만 얻었습니다.

## 1. 데이터 준비

세 전략의 5분봉, 15분봉 및 MTF 전략용 1시간봉을 동일한 시장과 기간으로 내려받습니다.
...
### 다중 타임프레임(MTF) 상위 지표 번인(Burn-in) 요건

`MultiTimeframeAtrStrategy`는 5분 기준봉과 1시간 상위 정보봉(Informative timeframe)을 함께 사용합니다:
- 상위 타임프레임에서 `ta.EMA(informative, timeperiod=200)`을 계산하므로 최소 200개 이상의 1시간봉(200시간 = 5분봉 환산 최소 2,400개)이 축적되어야 상위 EMA 200이 안정적으로 수렴합니다.
- 기준봉 설정인 `startup_candle_count = 799`(약 66.5시간)는 5분봉 지표에는 충분하지만, 백테스트 초기 구간이나 dry-run 시작 시 1시간봉 과거 데이터가 충분하지 않으면 `ema_200_1h`가 NaN으로 처리되어 진입 신호가 발생하지 않습니다.
- 따라서 MTF 전략 검증 시에는 최소 180일~365일 이상의 과거 데이터를 사전에 다운로드(`Get-MarketData.ps1 -Days 365`)한 후 충분한 웜업 기간을 두고 백테스트를 수행해야 합니다.
